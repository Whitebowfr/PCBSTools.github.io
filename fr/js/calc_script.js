...
function showAvailableMotherboard(cpuid, gpuid, slicf, motherboardlist) {
  for (mobo in motherboardlist.options) {
    ...
    if (
      data.procs[cpuid].socket ==
      data.motherboards[motherboardlist.options[mobo].innerHTML].socket
    ) {
      if (slicf == "1") {
        motherboardlist.options[mobo].style.display = "block";
      } else {
        if (
          data.motherboards[motherboardlist.options[mobo].innerHTML].multigpu !=
            null &&
          data.motherboards[
            motherboardlist.options[mobo].innerHTML
          ].multigpu.includes(data.gpus[gpuid].multigpu)
        ) {
          motherboardlist.options[mobo].style.display = "block";
        } else {
...
function scoreCalculatorCal() {
  ...
  if (!data.procs[cpu] || data.procs[cpu][ramc][rams] == "") {
    ...
    var score = getScore(
      ...
      data.gpus[gpu][slicf].score
    );
    var wattage = getWattage(
      data.gpus[gpu][slicf].wattage,
      data.procs[cpu].wattage,
      50
    );
    ...
    document.getElementById("cpuScoreResult1").innerText =
      data.procs[cpu][ramc][rams];
    document.getElementById("gpuScoreResult1").innerText =
      data.gpus[gpu][slicf].score;
...
function addSaveButton(target, id) {
  ...
  var text = document.createTextNode("Sélectionnez et enregistrez");
  button.append(text);
...
function generateBuild() {
  ...
  var results =
    Number(document.getElementById("buildForm").results1.value) || 10;
  ...
  for (cpu in data.procs) {
    if (data.procs[cpu].level <= level) {
      ...
      if (architecture == "skylake" && data.procs[cpu].type != "skylake") {
        ...
        data.procs[cpu].type != "kabylake"
      ) {
        continue;
      } else if (
        architecture == "coffeelake" &&
        data.procs[cpu].type != "coffeelake"
      ) {
        continue;
      } else if (
        architecture == "xseries" &&
        data.procs[cpu].type != "xseries"
      ) {
        continue;
      } else if (architecture == "ryzen" && data.procs[cpu].type != "ryzen") {
        continue;
      } else if (
        architecture == "threadripper" &&
        data.procs[cpu].type != "threadripper"
      ) {
        continue;
      }
          ...
          for (gpu in data.gpus) {
            ...
            for (slicf in data.gpus[gpu]) {
              ...
              var currentScore = getScore(
                data.procs[cpu][ramchannel][ramspeed],
                data.gpus[gpu][slicf].score
              );
              var currentPrice =
                data.procs[cpu].price + data.gpus[gpu][slicf].price;
              ...
              if (incmobo == true && incram == false) {
                for (mobo in data.motherboards) {
                  if (data.motherboards[mobo].level > level) {
                    ...
                    slicf == "2" &&
                    (data.motherboards[mobo].multigpu == null ||
                      data.motherboards[mobo].multigpu.includes(
                        data.gpus[gpu].multigpu
                      ) == false)
                  ) {
                    continue;
                  } else if (
                    data.motherboards[mobo].socket != data.procs[cpu].socket
                  ) {
                    continue;
                  } else if (
                    needoc == true &&
                    data.motherboards[mobo].oc != "Yes"
                  ) {
                    continue;
                  } else if (
                    data.motherboards[mobo].speeds.includes(
                      ramspeed.toString()
                    ) == false
                  ) {
                    continue;
                  }
                  var tempCurrentPrice =
                  ...
                  if (tempCurrentPrice <= budget - resbudget) {
                    ...
                    builds.push(
                      parts(
                        ...
                        pickedram || "-",
                        currentScore,
                        data.procs[cpu].wattage + data.gpus[gpu][slicf].wattage,
                        currentPrice,
                        budget - currentPrice
                      )
                    );
...
function upgradeBuild() {
  ...
  var currentRamChannel = document.getElementById("upgradeForm").currentRamC1
    .value;
  var currentRamSpeed = document.getElementById("upgradeForm").currentRamS1
    .value;
  ...
  var resbudget =
    Number(document.getElementById("upgradeForm").resbudget2.value) || 0;
  ...
  var results =
    Number(document.getElementById("upgradeForm").results2.value) || 10;
  var upgrades = [];
  if (offset == "") {
    offset = 200;
  } else {
  ...
  var currentScore = getScore(
    data.procs[currentProc][currentRamChannel][currentRamSpeed],
  ...
  for (cpu in data.procs) {
    ...
    var newScore = getScore(
      data.procs[cpu][currentRamChannel][currentRamSpeed],
      data.gpus[currentGpu][currentSlicf].score
    );
    if (newScore >= score && newScore < score + offset) {
      upgrades.push(
        parts(
          ...
          "-",
          newScore,
          data.procs[cpu].wattage + data.gpus[currentGpu][currentSlicf].wattage,
          cost,
          budget - cost
        )
      );
  ...
  for (gpu in data.gpus) {
    ...
    for (sf in data.gpus[gpu]) {
      ...
      if (
        sf == "2" &&
        data.motherboards[currentMobo].multigpu != null &&
        data.motherboards[currentMobo].multigpu.includes(
          data.gpus[gpu].multigpu
        ) == false
      ) {
        continue;
      ...
      if (newScore >= score && newScore < score + offset) {
        upgrades.push(
          parts(
            ...
            "-",
            newScore,
            data.procs[currentProc].wattage + data.gpus[gpu][sf].wattage,
            cost,
            budget - cost
          )
        );
  ...
  if (
    data.motherboards[currentMobo].multigpu != null &&
    data.motherboards[currentMobo].multigpu.includes(
      data.gpus[currentGpu].multigpu
    )
  ) {
    var cost = data.gpus[currentGpu]["1"].price;
    var newScore = getScore(
      data.procs[currentProc][currentRamChannel][currentRamChannel],
      data.gpus[currentGpu]["2"]
    );
    if (cost > budget - resbudget) {
      if (newScore >= score && newScore < score + offset) {
        upgrades.push(
          parts(
            ...
            "-",
            newScore,
            data.procs[currentProc].wattage +
              data.gpus[currentGpu]["2"].wattage,
            cost,
            budget - cost
          )
        );
          ...
          if (newScore >= score && newScore < score + offset) {
            upgrades.push(
              parts(
                ...
                "-",
                newScore,
                data.procs[cpu].wattage + data.gpus[gpu][sf].wattage,
                cost,
                budget - cost
              )
            );
